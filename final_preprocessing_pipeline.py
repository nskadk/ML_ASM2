...
Original file is located at
    https://colab.research.google.com/drive/1VMH1myMbPC2UyuISzjZot_dks15o1mEa
"""
...
# # Extract the zip file to the specified path in Google Drive
# with zipfile.ZipFile('./drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/preprocessed_test_images.zip', 'r') as zip_ref:
#     zip_ref.extractall('./drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/preprocessed_test_images')

# print(f"Files extracted successfully to")
...
class PreprocessingPipeline:
    ...
    def _remove_duplicates(self):
        ...
        if num_duplicates > 0:
            ...
            with open(os.path.join(self.config['output_dir'], 'analysis', 'duplicates.json'), 'w') as f:
            ...
            if hasattr(self.dataset, 'labels') and len(self.dataset.labels) > 0:
                new_labels = [self.dataset.labels[path_to_idx[path]] for path in unique_paths]
                self.dataset.labels = new_labels

                # Update label indices if they exist
                if hasattr(self.dataset, 'label_indices') and len(self.dataset.label_indices) > 0:
                    self.dataset.label_indices = [self.dataset.class_to_idx.get(label, 0)
    ...
    def preprocess_single_image(self, image_path):
        ...
        if self.pipeline is None:
            logger.error("Pipeline not initialized. Run the full pipeline first.")
            return None
        ...
        try:
            # Apply the same preprocessing steps as the pipeline
    ...
    def preprocess_folder(self, input_folder, output_folder, recursive=True, maintain_structure=True):
        """Preprocess all images in a folder using the current pipeline or creating a new one

        Args:
            input_folder: Path to folder containing images
            output_folder: Path to save preprocessed images
            recursive: Whether to process subfolders (default: True)
            maintain_structure: Whether to maintain folder structure (default: True)
        ...
        # Check if pipeline exists, if not, analyze the folder to create one
        if self.pipeline is None:
            logger.info("Pipeline not initialized. Analyzing images to determine preprocessing steps...")

            # Create a temporary aggregator
            ...
            with ThreadPoolExecutor(max_workers=self.config.get('num_workers', 4)) as executor:
                ...
                for path in sample_paths:
                    futures.append(executor.submit(self._analyze_path, path))

                for future in tqdm(futures, total=len(futures), desc="Analyzing samples"):
                    result, error = future.result()
                    temp_aggregator.add_result(result, error)

            # Generate report and determine pipeline
            folder_stats = temp_aggregator.generate_report()
            self.pipeline, self.pipeline_description = self.preprocessor.determine_pipeline(folder_stats)
    ...
    def preprocess_single_image(self, image_path):
        """Preprocess a single image using the current pipeline or creating a new one
...
###################################################################################
#       UNCOMMENT THIS ONLY IF YOU WANTED TO PREPROCESSING THE TRAIN DATASET      #
###################################################################################


# # Define paths
# dataset_path = './drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/train_images'
# preprocessed_dir = './drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/processed_dataset_testing'
...
# input_folder = "./drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/test_images"
# output_folder = "./drive/MyDrive/Colab Notebooks/Machine Learning - ASM 3/preprocessed_test_images"
...
###################################################################################
#       UNCOMMENT THIS ONLY IF YOU WANTED TO PREPROCESSING THE INPUT IMAGE        #
###################################################################################
